...
class Detection:
    ...
    def generate_features(self, d):
        ...
        # print("len col f")
        # print(len(columns_features_list))
    ...
    def predict_labels(self, d):
        ...
        for j in range(d.dataframe.shape[1]):
            ...
            if sum(y_train) == 0:
                num_ones = len(y_train) * 10 // 100
                # Get random indices for the ones
                ones_indices = random.sample(range(len(y_train)), num_ones)
                # Set ones at those indices
    ...
    def run(self, dd):
        ...
        end_time = time.time()
        if self.VERBOSE:
            print("The feature generation run time is {} seconds.".format(end_time - start_time))
        start_time = time.time()
        ...
        end_time = time.time()
        if self.VERBOSE:
            print("The clustering run time is {} seconds.".format(end_time - start_time))
...
if __name__ == "__main__":
    ...
    dataset_dictionary = {
        "name": dataset_name,
        "path": os.path.abspath(os.path.join(dataset_path, "dirty.csv")),
